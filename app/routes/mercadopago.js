...
router.get('/authorization', (req, res) => {
  ...
  mercadopagoService.processAuthorization(code, commerceId)
    .then(() => res.redirect(constants.MERCADOPAGO.SUCCESS_URL.replace('{commerceId}', commerceId)))
    .catch(({data, response}) => {
...
router.post('/notifications', async (req, res) => {
  ...
  if (accountId.toString() !== config.MERCADOPAGO_AM_USER_ID) {
    try {
      const commerceAccount = await paymentService.findCommerceByAccountId(accountId)
  ...
  mercadopagoService.getOrderByResource(type, resourceId, accessToken)
